...
class EmergencyAIObserverTests(unittest.TestCase):
    ...
    def test_non_positive_limits_never_call_provider(self):
        ...
        result = EmergencyAIObserver(ai).analyze_event(sample_event(), max_summary_chars=0)
        self.assertFalse(result.ok)
        self.assertEqual(result.status, "error")
        self.assertEqual(ai.calls, 0)

    def test_valid_json_returns_shadow_analysis(self):
        response = AIResult(ok=True, status="available", duration_ms=17, text=json.dumps({
    ...
    def test_summary_and_notes_respect_exact_limits(self):
        ...
        ai = FakeAI(enabled=True, emergencies=True, response=response)
        result = EmergencyAIObserver(ai).analyze_event(sample_event(), max_summary_chars=7, max_notes_chars=10)
        self.assertTrue(result.ok)
    ...
    def test_summary_prefers_complete_sentence_when_truncated(self):
        ...
        ai = FakeAI(enabled=True, emergencies=True, response=response)
        result = EmergencyAIObserver(ai).analyze_event(sample_event(), max_summary_chars=40, max_notes_chars=100)
        self.assertTrue(result.ok)
    ...
    def test_decimal_point_is_not_treated_as_sentence_end(self):
        response = AIResult(ok=True, status="available", text=json.dumps({
            "summary": "Posible foco detectado por NASA FIRMS en Zaragoza con FRP total de 18.4 MW y extensión observada de 1.2 km en seguimiento satelital.",
            "notes": "Sin confirmación de terreno.",
            "confidence": 0.8,
        }))
        ai = FakeAI(enabled=True, emergencies=True, response=response)
        result = EmergencyAIObserver(ai).analyze_event(sample_event(), max_summary_chars=95, max_notes_chars=100)
        self.assertTrue(result.ok)
    ...
    def test_invalid_coordinates_are_not_forwarded_as_strings(self):
        ...
        ai = FakeAI(enabled=True, emergencies=True, response=response)
        result = EmergencyAIObserver(ai).analyze_event(sample_event(latitude="no-num", longitude=""), change="updated")
        self.assertTrue(result.ok)
        payload = json.loads(ai.last_prompt)
    ...
    def test_invalid_json_is_rejected(self):
        ai = FakeAI(enabled=True, emergencies=True, response=AIResult(ok=True, status="available", text="no-json"))
    ...
    def test_empty_summary_is_rejected(self):
        ai = FakeAI(enabled=True, emergencies=True, response=AIResult(ok=True, status="available", text=json.dumps({
            "summary": "", "notes": "x", "confidence": 0.5
        })))
        result = EmergencyAIObserver(ai).analyze_event(sample_event())
        self.assertFalse(result.ok)
        self.assertEqual(result.status, "error")
    ...
    def test_provider_failure_is_propagated_as_safe_fallback(self):
        ai = FakeAI(enabled=True, emergencies=True, response=AIResult(ok=False, status="degraded", error="proveedor IA no accesible", duration_ms=25))
        result = EmergencyAIObserver(ai).analyze_event(sample_event())
        self.assertFalse(result.ok)
    ...
    def test_deterministic_phase_reuses_firms_tracking(self):
        ...
        self.assertEqual(deterministic_phase(sample_event(metadata={"firms_phase": "stable"}), "updated"), "stable")
    ...
    def test_non_firms_source_cannot_inherit_firms_phase_metadata(self):
        ...
        event = sample_event(source="aemet", metadata={"firms_phase": "growth"}, category="storm")
        self.assertEqual(deterministic_phase(event, "updated"), "updated")
    ...
    def test_event_is_not_mutated(self):
        ...
        ai = FakeAI(enabled=True, emergencies=True, response=AIResult(ok=True, status="available", text=json.dumps({
            "summary": "Aviso por viento.", "notes": "", "confidence": 0.9
        })))
