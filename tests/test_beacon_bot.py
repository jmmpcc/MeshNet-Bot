...
# Stub mínimo para importar beacon_bot sin depender del repo completo en esta prueba aislada.
class _Caps:
...
class BeaconBotTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_validate_definition(self):
        parsed = beacon_bot._validate_definition(["10", "3", "guardia", "2", "Texto", "de", "prueba"])
        self.assertEqual(parsed, (10, 3, "guardia", 2, "Texto de prueba"))
    ...
    def test_send_meshtastic_uses_existing_broker_route(self):
    ...
    def test_send_meshcore_uses_existing_broker_route(self):
        spec = beacon_bot.BeaconSpec("meshcore", 10, 1, "mc", 4, "hola mc")
        with patch.object(beacon_bot, "_broker_rpc", return_value={"ok": True}) as rpc:
            result = beacon_bot._send_beacon_sync(spec)
        self.assertTrue(result["ok"])
        cmd, params = rpc.call_args.args
        ...
        self.assertEqual(params["text"], "hola mc")

    async def test_start_and_stop_by_name(self):
        update = self._update()
        context = SimpleNamespace(args=["60", "1", "prueba", "1", "texto"])
    ...
    def test_contextual_help_respects_profile(self):
        with patch.object(beacon_bot, "_available_transports", return_value={"meshcore"}):
