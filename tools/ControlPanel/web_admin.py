...
def create_app(registry: ToolRegistry | None = None, auth_token: str | None = None) -> FastAPI:
    ...
    def set_emergency_collection(payload: EmergencyCollectionPayload) -> dict[str, Any]:
        ...
        if "aemet_cap" in sources and not provinces:
            raise HTTPException(status_code=422, detail="AEMET CAP requiere al menos una provincia seleccionada")
        if "che_saih" in sources and not provinces:
            raise HTTPException(status_code=422, detail="CHE / SAIH requiere al menos una provincia seleccionada")
        ...
        if firms_key:
            secret_updates["FIRMS_MAP_KEY"] = firms_key
        if aemet_key:
            secret_updates["AEMET_API_KEY"] = aemet_key
        if secret_updates:
            try:
                update_env_values(EMERGENCIAS_ENV_FILE, secret_updates)
            except OSError as exc:
                raise HTTPException(status_code=500, detail=f"No se pudieron guardar las claves de fuentes: {exc}") from exc
...
const auditTransportLabels={meshcore:'MeshCore',meshtastic:'Meshtastic',aprs:'APRS',aprs_rf:'APRS RF',aprsis:'APRS-IS',voice_rf:'Voz RF'};
