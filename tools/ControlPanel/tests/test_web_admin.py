...
def test_systemd_service_listens_on_lan_with_authentication_environment():
    unit = (web_admin.BASE_DIR / "systemd/meshnet-control-panel.service").read_text()
    assert "Environment=CONTROLPANEL_HOST=0.0.0.0" in unit
    assert "EnvironmentFile=-/home/meshnet/MeshNet-Bot/tools/ControlPanel/.env" in unit
