...
function sync(data){{ if(data.revision===revision){{document.getElementById('updated').textContent=fmt(data.generated_at);return;}} revision=data.revision||''; const alive=new Set(); for(const e of data.events||[]){{alive.add(e.event_id); const pos={{lat:Number(e.latitude),lng:Number(e.longitude)}}; let m=markers.get(e.event_id); if(!m){{m=new google.maps.Marker({{position:pos,map,title:markerTitle(e)}}); const info=new google.maps.InfoWindow(); m.addListener('click',()=>{{info.setContent(popup(e));info.open({{map,anchor:m}})}}); markers.set(e.event_id,m);}} else {{m.setPosition(pos);m.setTitle(markerTitle(e));}}}} for(const [id,m] of markers){{if(!alive.has(id)){{m.setMap(null);markers.delete(id);}}}} document.getElementById('count').textContent=`${{(data.events||[]).length}} activas`; document.getElementById('updated').textContent=fmt(data.generated_at||data.source_updated_at);}}
