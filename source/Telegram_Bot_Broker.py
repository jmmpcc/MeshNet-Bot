...
def _send_aprs_immediate(dest: str, text: str, timeout: float = 8.0) -> dict:
    """
    Envía una orden APRS al gateway UDP y espera su respuesta real.

    Antes el bot troceaba el texto localmente y daba OK cuando el datagrama UDP
    salía del socket. Eso podía mostrar "OK (3/3 partes)" aunque el gateway no
    hubiese conseguido transmitir por KISS/RF. El gateway ya sabe trocear, aplicar
    deduplicación y devolver parts/sent, así que aquí se envía una sola orden con
    el texto completo y se informa del resultado confirmado por la pasarela.
    """
    ...
    ctrl = {"mode": "aprs", "dest": aprs_dest, "text": text_clean}
    if APRS_BOT_PATH:
        ctrl["path"] = APRS_BOT_PATH
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            try:
                ...
                if isinstance(resp, dict):
                    parts = int(resp.get("parts", resp.get("chunks", 0)) or 0)
                    sent = int(resp.get("sent", parts if resp.get("ok") else 0) or 0)
                    return {
                        "ok": bool(resp.get("ok")),
                ...
                return {"ok": False, "dest": aprs_dest, "chunks": 0, "sent": 0, "error": "aprs_gateway_timeout"}
    except Exception as e:
        return {"ok": False, "dest": aprs_dest, "chunks": 0, "sent": 0, "error": f"{type(e).__name__}: {e}"}
    return {"ok": False, "dest": aprs_dest, "chunks": 0, "sent": 0, "error": "invalid_gateway_response"}
