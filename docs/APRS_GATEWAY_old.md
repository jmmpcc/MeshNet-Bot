...
### 2️⃣ Mensajes APRS → Meshtastic (downlink)
      ...
      - ## v6.1.3 – Integración completa APRS→Mesh (2025-02)
        ### Nueva funcionalidad

      -  **Envío inmediato desde APRS a Meshtastic** mediante etiquetas:
           - `[CH n] texto`
           - `[CANAL n] texto`
      - **Programación de mensajes vía APRS** usando:
          - `[CH n+M] texto` → M minutos de retraso
      - **Heurística para compatibilidad APRS “colapsada”**:
          - `[CHXY]` cuando `XY > 15` → `canal = X`, `delay = Y`
      - **Control total del gateway APRS → Mesh desde RF**:
          - `[CH0] APRS ON`
          - `[CH0] APRS OFF`
      - **Conversión automática de posiciones APRS** en enlaces clicables:
          - `https://maps.google.com/?q=lat,lon`
      - **Limpieza automática del prefijo `[CH…]`** en el mensaje final recibido en Mesh.
      - **Integración con el broker Meshtastic**:
          - Envío inmediato → `_broker_send_text`
           - Programado → `_schedule_aprs_to_mesh`
       -  
